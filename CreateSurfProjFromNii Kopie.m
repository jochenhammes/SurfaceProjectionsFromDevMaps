...
LateralProjection1(imgContourLat > 0) = maxVoxelValue;
LateralProjection2(imgContourLat > 0) = maxVoxelValue;
LateralProjection3(imgContourLat > 0) = maxVoxelValue;
LateralProjection4(imgContourLat > 0) = maxVoxelValue;
AxialProjection1(imgContourTop > 0) = maxVoxelValue;
AxialProjection2(imgContourTop > 0) = maxVoxelValue;

%% Compose Output Image
%DisplayProjections = vertcat(imrotate(LateralProjection1,90), imrotate(LateralProjection3,90), fliplr(imrotate(LateralProjection2,90)), fliplr(imrotate(LateralProjection4,90)));
...
imgLR = imread('LR.bmp');
imgLR(imgLR > 0) = 1;
imgLR = imgLR / max(imgLR(:)) * max(DisplayProjections(:));
...
horizontalOffset = round(size(DisplayProjections, 2)/6);

% DisplayProjections((1+verticalOffset):(14+verticalOffset),(1+horizontalOffset):(horizontalOffset+20))=imgLR(:,1:20);
% DisplayProjections((1+verticalOffset):(14+verticalOffset),(1+horizontalOffset*3):(horizontalOffset*3+20))=imgLR(:,24:43);
% DisplayProjections((1+verticalOffset):(14+verticalOffset),(1+horizontalOffset*5-15):(horizontalOffset*5+43-15))=imgLR(:,1:43);
...
DisplayProjections = AddTextToImage(DisplayProjections,'L', [verticalOffset horizontalOffset], textColor,'Arial', 30);
DisplayProjections = AddTextToImage(DisplayProjections,'R', [verticalOffset 3*horizontalOffset], textColor,'Arial', 30);
DisplayProjections = AddTextToImage(DisplayProjections,'L   R', [verticalOffset ceil(4.75*horizontalOffset)], textColor,'Arial', 30);

%Draw Colorbar to image
ColorBar = 1:256;
ColorBar = ColorBar/256*maxVoxelValue;

ColRangeColorBar = (size(DisplayProjections,2)/2-128):(size(DisplayProjections,2)/2+127);
...
%DisplayProjections = AddTextToImage(DisplayProjections,num2str(round(maxVoxelValue, 1)), [RowRangeColorBar(1)-10, ColRangeColorBar(end)+10], textColor,'Arial', 20);
DisplayProjections = AddTextToImage(DisplayProjections,sprintf('%.1f',maxVoxelValue), [RowRangeColorBar(1)-10, ColRangeColorBar(end)+10], textColor,'Arial', 20);
...
%Create outputs Mono-Bitmap, RGB-Bitmap, Nifti
